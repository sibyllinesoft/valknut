...
impl DirectoryHealthTree {
    ...
    pub fn from_candidates(refactoring_candidates: &[RefactoringCandidate]) -> Self {
        ...
        for candidate in refactoring_candidates {
            ...
            if let Some(dir_path) = file_path.parent() {
                ...
                directory_data.entry(dir_path.clone()).or_default().push(candidate);
        ...
        for dir in &all_directories {
            ...
            let (total_issues, health_score) = if dir_candidates.is_empty() {
                ...
                let has_children_with_issues = directory_data.keys()
                ...
                let avg_score = dir_candidates.iter().map(|c| c.confidence).sum::<f64>() / total_issues as f64;
            ...
            let depth_stat = depth_stats.entry(depth).or_insert_with(|| DepthHealthStats {
                depth,
                directory_count: 0,
            ...
            for candidate in dir_candidates {
                for issue in &candidate.issues {
                    let summary = issue_categories.entry(issue.category.clone()).or_insert_with(|| {
            ...
            let dir_health = DirectoryHealthScore {
                ...
                critical_issues: dir_candidates.iter()
                    .flat_map(|c| &c.issues)
                    .filter(|issue| issue.severity >= 2.0)
                    .count(),
                high_priority_issues: dir_candidates.iter()
                    .flat_map(|c| &c.issues)
                    .filter(|issue| issue.severity >= 1.5)
                    .count(),
                avg_refactoring_score: if dir_candidates.is_empty() { 0.0 } else {
                    dir_candidates.iter().map(|c| c.score).sum::<f64>() / dir_candidates.len() as f64
                },
                weight: 1.0,
        ...
        for dir in &all_directories {
            let children: Vec<PathBuf> = all_directories.iter()
                .filter(|other_dir| {
                    other_dir.parent() == Some(dir.as_path()) 
                })
                .cloned()
                .collect();
        ...
        // Find root directory
        let root_path = all_directories.iter().min_by_key(|p| p.components().count()).cloned()
            .unwrap_or_else(|| PathBuf::from("."));
        
        let root = directories.get(&root_path).cloned().unwrap_or_else(|| DirectoryHealthScore {
            path: root_path,
            health_score: 1.0,
            file_count: 0,
            entity_count: 0,
        ...
        let tree_statistics = TreeStatistics {
            ...
            max_depth: 1,
            avg_health_score: if directories.is_empty() { 1.0 } else {
    ...
    fn append_directory_tree(&self, result: &mut String, dir: &DirectoryHealthScore, depth: usize) {
        ...
        // Add children
        let mut children: Vec<_> = dir.children.iter()
...
impl AnalysisResults {
    ...
    pub fn group_candidates_by_file(candidates: &[RefactoringCandidate]) -> Vec<FileRefactoringGroup> {
        use std::collections::HashMap;
        let mut file_groups: HashMap<String, Vec<RefactoringCandidate>> = HashMap::new();
        
        // Group candidates by file path
        for candidate in candidates {
            file_groups.entry(candidate.file_path.clone())
                .or_insert_with(Vec::new)
                .push(candidate.clone());
        }
        
        // Convert to FileRefactoringGroup structs
        let mut groups: Vec<FileRefactoringGroup> = file_groups.into_iter()
            .map(|(file_path, entities)| {
                ...
                let avg_score = if entities.is_empty() { 0.0 } else {
                    entities.iter().map(|e| e.score).sum::<f64>() / entities.len() as f64
                };
                
                // Find highest priority 
                let highest_priority = entities.iter()
                    .map(|e| &e.priority)
                    .max()
                    .cloned()
                ...
                // Count total issues
                let total_issues = entities.iter()
                    .map(|e| e.issues.len())
                    .sum();
        ...
        groups.sort_by(|a, b| {
            ...
            if priority_cmp != std::cmp::Ordering::Equal {
                ...
                b.avg_score.partial_cmp(&a.avg_score).unwrap_or(std::cmp::Ordering::Equal)
            }
        });
    ...
    pub fn from_pipeline_results(pipeline_results: PipelineResults) -> Self {
        ...
        let refactoring_candidates_by_file = Self::group_candidates_by_file(&refactoring_candidates);
        ...
        let directory_health_tree = Self::build_directory_health_tree(
            &pipeline_results, 
    ...
    fn build_directory_health_tree(
        ...
        if let Some(first_dir) = all_directories.iter().next() {
            ...
            for dir in all_directories.iter().skip(1) {
                ...
                let common_len = root_components.iter()
        ...
        for dir_path in &all_directories {
            ...
            let critical_issues = candidates_in_dir.iter().filter(|c| matches!(c.priority, Priority::Critical)).count();
            let high_priority_issues = candidates_in_dir.iter().filter(|c| matches!(c.priority, Priority::High | Priority::Critical)).count();
            ...
            if dir_path.as_os_str() == "src" {
                println!("DEBUG: SRC calculation - entities: {}, refactoring: {}, avg_score: {}", total_entity_count, refactoring_needed, avg_refactoring_score);
            }
            ...
            for candidate in &candidates_in_dir {
                for issue in &candidate.issues {
                    ...
                    summary.max_severity = summary.max_severity.max(issue.severity);
                    summary.health_impact = summary.avg_severity * (summary.affected_entities as f64 / total_entity_count as f64);
                }
            }
            ...
            let children: Vec<PathBuf> = all_directories
                .iter()
                .filter(|other_dir| {
                    other_dir.parent() == Some(dir_path)
                })
                .cloned()
                .collect();
        ...
        let root = directories.get(&root_path).cloned().unwrap_or_else(|| {
            DirectoryHealthScore {
                ...
                avg_refactoring_score: 0.0,
                weight: 1.0,
                children: directories.keys().filter(|p| p != &&root_path).cloned().collect(),
                parent: None,
                issue_categories: HashMap::new(),
            }
        });
        ...
        let max_depth = directories.keys()
        ...
        let health_score_std_dev = if health_scores.len() > 1 {
            let variance = health_scores.iter()
                ...
                .sum::<f64>() / (health_scores.len() - 1) as f64;
            variance.sqrt()
        } else {
            0.0
        };
        ...
        let mut hotspot_candidates: Vec<_> = directories.values()
        ...
        let hotspot_directories: Vec<DirectoryHotspot> = hotspot_candidates
            ...
            .enumerate()
            .map(|(rank, dir)| {
                let primary_issue_category = dir.issue_categories
                    .values()
                ...
                let recommendation = Self::generate_hotspot_recommendation(&primary_issue_category, dir);
        ...
        for dir in directories.values() {
            ...
            let depth_stats = health_by_depth.entry(depth).or_insert_with(|| {
                DepthHealthStats {
                    depth,
                    directory_count: 0,
                    avg_health_score: 0.0,
        ...
        println!("DEBUG: DirectoryHealthTree has {} directories", directories.len());
        for (path, score) in &directories {
            println!("DEBUG: Directory {:?} has health {:.1}%, children: {:?}", path, score.health_score * 100.0, score.children);
        }
    ...
    fn convert_lsh_to_clone_analysis(pipeline_results: &PipelineResults) -> Option<CloneAnalysisResults> {
        ...
        Some(CloneAnalysisResults {
            ...
            performance_metrics: CloneAnalysisPerformance {
                ...
                memory_usage_bytes: pipeline_results.statistics.memory_stats.peak_memory_bytes as u64,
                entities_per_second: if pipeline_results.statistics.total_duration_ms > 0 {
                    (pipeline_results.results.summary.total_entities as f64 * 1000.0) / pipeline_results.statistics.total_duration_ms as f64
                } else {
                    0.0
                },
            },
    ...
    fn convert_coverage_to_packs(coverage_results: &crate::core::pipeline::CoverageAnalysisResults) -> Vec<crate::detectors::coverage::CoveragePack> {
    ...
    fn generate_hotspot_recommendation(primary_issue_category: &str, dir: &DirectoryHealthScore) -> String {
...
impl RefactoringCandidate {
    ...
    fn generate_suggestions(issues: &[RefactoringIssue]) -> Vec<RefactoringSuggestion> {
        ...
        for issue in issues {
            match issue.category.as_str() {
                "complexity" => {
                    ...
                    let mut complexity_features = issue.contributing_features.iter()
                        .filter(|f| f.feature_name.contains("complexity"))
                        .collect::<Vec<_>>();
                    complexity_features.sort_by(|a, b| b.contribution.partial_cmp(&a.contribution).unwrap_or(std::cmp::Ordering::Equal));
                    
                    if issue.severity >= 2.0 {
                        let primary_feature = complexity_features.first()
                    ...
                    if issue.severity >= 1.5 {
                        ...
                        let conditional_contribution = issue.contributing_features.iter()
                            .filter(|f| f.feature_name.contains("conditional") || f.feature_name.contains("branch"))
                ...
                "structure" => {
                    ...
                    let structural_features = issue.contributing_features.iter()
                        .collect::<Vec<_>>();
                    
                    let description = if !structural_features.is_empty() {
                        let primary_issues = structural_features.iter()
                            .take(2)
                            .map(|f| format!("{} ({})", 
        ...
        suggestions.dedup_by(|a, b| a.refactoring_type == b.refactoring_type && a.description == b.description);
