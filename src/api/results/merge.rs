//! Merge operations for combining analysis results from parallel processing.
//!
//! This module provides merge implementations for analysis result types,
//! enabling efficient combination of results from parallel analysis runs.
//! All merge operations use weighted averaging where appropriate to preserve
//! statistical accuracy.

use super::merge_helpers::{
    merge_count_maps, merge_health_metrics, merge_maps, merge_optional_average,
    merge_optional_bool, merge_optional_max_u64, merge_optional_sum, merge_optional_sum_u64,
    merge_stage_results, weighted_average, weighted_duration,
};
use super::models::{
...
/// Merge operations for combining [`AnalysisResults`] from parallel runs.
impl AnalysisResults {
    /// Consumes self and merges with another result, returning the combined result.
    pub fn merge(mut self, other: AnalysisResults) -> Self {
        self.merge_in_place(other);
        self
    }

    /// Merges another result into self, modifying self in place.
    ///
    /// Combines all metrics using weighted averaging where appropriate,
    /// and accumulates collections like candidates and warnings.
...
/// Merge operations for clone analysis results.
impl CloneAnalysisResults {
    /// Merges another clone analysis result into self.
    ///
    /// Combines denoising statistics, thresholds, and quality scores
    /// using weighted averaging based on candidate counts.
...
/// Merge operations for phase filtering statistics.
impl PhaseFilteringStats {
    /// Merges phase filtering counts by summing each phase's count.
...
/// Merge operations for clone analysis performance metrics.
impl CloneAnalysisPerformance {
    /// Merges performance metrics by summing time and taking max memory.
...
// Helper functions extracted to merge_helpers.rs

#[cfg(test)]
mod tests {
    use super::*;
    use crate::api::results::merge_helpers::{
        merge_health_metrics, merge_optional_average, merge_optional_bool, merge_optional_max_u64,
        merge_optional_sum, merge_optional_sum_u64, weighted_average, weighted_duration,
    };
