...
use super::super::common::{EntityKind, LanguageAdapter, ParseIndex, ParsedEntity, SourceLocation};
...
/// Parsing and entity extraction methods for [`RustAdapter`].
impl RustAdapter {
    ...
    fn has_modifier(node: &Node, modifier_kind: &str) -> bool {
        ...
        for child in node.children(&mut cursor) {
            ...
            if child.kind() == "function_modifiers" && Self::has_child_kind(&child, modifier_kind) {
                return true;
            }
        }
        false
    ...
    /// Check if a node has a child of the given kind.
    ...
    /// Collect child text of the specified kind from a node.
    fn collect_child_text<'a>(node: &Node, source_code: &'a str, kind: &str) -> Vec<&'a str> {
        let mut cursor = node.walk();
    ...
    fn extract_nested_identifiers<'a>(
        ...
        for child in parent.children(&mut cursor) {
            if child.kind() == outer_kind {
                results.extend(Self::collect_child_text(&child, source_code, inner_kind));
            }
        }
        Ok(results)
    }
...
impl LanguageAdapter for RustAdapter {
    ...
    /// Extracts all function and macro call targets from the source.
    fn extract_function_calls(&mut self, source: &str) -> Result<Vec<String>> {
        ...
        Self::walk_tree(tree.root_node(), &mut |node| {
            ...
            let Ok(text) = Self::node_text(&candidate, source) else { return };
    ...
    fn extract_identifiers(&mut self, source: &str) -> Result<Vec<String>> {
        ...
        Self::walk_tree(tree.root_node(), &mut |node| {
            if !matches!(
                node.kind(),
                "identifier" | "field_identifier" | "type_identifier" | "scoped_identifier" | "lifetime"
            ) {
                return;
            }
            let Ok(text) = Self::node_text(&node, source) else { return };
            let cleaned = text.trim();
            if !cleaned.is_empty() {
    ...
    /// Extracts use statements and mod declarations from Rust source.
...
impl RustAdapter {
    ...
    fn try_parse_mod_declaration(trimmed: &str) -> Option<&str> {
        ...
        let mod_name = mod_part.strip_suffix(';')?.trim();
    ...
    fn parse_use_statement(use_part: &str, line_number: usize, imports: &mut Vec<ImportStatement>) {
        if let Some(stmt) = Self::parse_grouped_use(use_part, line_number) {
            imports.push(stmt);
        } else {
            imports.push(Self::create_simple_use(use_part, line_number));
        }
    }
    ...
    fn parse_grouped_use(use_part: &str, line_number: usize) -> Option<ImportStatement> {
        ...
        let close_brace = items_part.find('}')?;
    ...
    /// Create a simple use import statement
    fn create_simple_use(use_part: &str, line_number: usize) -> ImportStatement {
        ImportStatement {
    ...
    /// Create a mod import statement
    fn create_mod_import(mod_name: &str, line_number: usize) -> ImportStatement {
        ImportStatement {
...
/// Default implementation for [`RustAdapter`].
impl Default for RustAdapter {
    /// Returns a new Rust adapter, or a minimal fallback on failure.
    fn default() -> Self {
