...
impl CallGraph {
    ...
    fn add_aggregated_edge(&mut self, edge: &AggregatedEdge) -> Result<()> {
        ...
        let edge_index = if let Some(edge_idx) = self.graph.find_edge(caller_node, callee_node) {
            ...
            self.graph.add_edge(caller_node, callee_node, MultiEdge::default())
        };
        
        // Update edge data
        let edge_weight = self.graph.edge_weight_mut(edge_index)
        ...
        match edge.kind {
            ...
            EdgeKind::Static => {
                ...
                edge_weight.static_first_seen = Some(
                    edge_weight.static_first_seen
                        .unwrap_or(edge.first_timestamp())
                        .min(edge.first_timestamp())
                );
                edge_weight.static_last_seen = Some(
                    edge_weight.static_last_seen
                        .unwrap_or(edge.last_timestamp())
                        .max(edge.last_timestamp())
                );
            },
                    ...
                    if edge_weight.runtime_calls > 0 {
                        ...
                        if let Some(first_seen) = edge_weight.runtime_first_seen {
                            target_stats.first_seen = Some(
                                target_stats.first_seen.unwrap_or(first_seen).min(first_seen)
                            );
                        }
                        if let Some(last_seen) = edge_weight.runtime_last_seen {
                            target_stats.last_seen = Some(
                                target_stats.last_seen.unwrap_or(last_seen).max(last_seen)
                            );
                        }
                    }
                }
    ...
    fn compute_seed_reachability(&mut self, static_weight: f64) -> Result<()> {
        ...
        if queue.is_empty() {
            let mut node_scores: Vec<_> = self.node_stats.iter()
    ...
    pub fn get_stats(&self) -> GraphStats {
        ...
        let isolated_nodes = self.graph.node_indices()
            .filter(|&node_idx| {
                self.graph.edges(node_idx).count() == 0 &&
                self.graph.edges_directed(node_idx, petgraph::Direction::Incoming).count() == 0
            })
            .count();
    ...
    pub fn get_node_stats(&self, symbol: &str) -> Option<&NodeStats> {
        self.symbol_to_node.get(symbol)
    ...
    pub fn iter_nodes(&self) -> impl Iterator<Item = (&str, &NodeStats)> {
        self.graph.node_weights().zip(self.graph.node_indices())
            .filter_map(|(symbol, node_idx)| {
                self.node_stats.get(&node_idx).map(|stats| (symbol.as_str(), stats))
            })
    }
...
mod tests {
    ...
    fn create_test_edge_with_timestamps(caller: &str, callee: &str, kind: EdgeKind, calls: u64, first_ts: u64, last_ts: u64) -> AggregatedEdge {
        AggregatedEdge {
            caller: caller.to_string(),
            callee: callee.to_string(),
            kind,
    ...
    fn test_undirected_projection_comprehensive() -> Result<()> {
        ...
        assert_eq!(undirected.get_symbol(undirected.get_node_index("a").unwrap()), Some("a"));
    ...
    fn test_timestamp_handling() -> Result<()> {
        ...
        let edges = vec![
            create_test_edge_with_timestamps("a", "b", EdgeKind::Runtime, 10, now_ts - 3600, now_ts),
            create_test_edge_with_timestamps("a", "c", EdgeKind::Runtime, 5, now_ts - 7200, now_ts - 1800), // Changed to Runtime
        ];
