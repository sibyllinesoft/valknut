...
impl LouvainDetector {
    ...
    fn calculate_node_modularity_contribution(
        ...
        let modularity_gain = (edges_to_community - self.resolution * expected_edges) / total_weight;
        
        Ok(modularity_gain)
    }
    
    /// Calculate total weight of all edges in graph
    fn calculate_total_weight(&self, graph: &petgraph::Graph<String, f64, petgraph::Undirected>) -> f64 {
        graph.edge_weights().sum()
    }
    
    /// Calculate degree (sum of edge weights) for a node
    fn calculate_node_degree(&self, graph: &petgraph::Graph<String, f64, petgraph::Undirected>, node: NodeIndex) -> f64 {
...
mod tests {
    ...
    fn test_empty_graph() {
        ...
        let modularity = detector.calculate_modularity(&empty_graph, &node_to_community).unwrap();
    ...
    fn test_single_node_graph() {
        ...
        let modularity = detector.calculate_modularity(&graph, &node_to_community).unwrap();
    ...
    fn test_two_node_connected_graph() {
        ...
        let same_modularity = detector.calculate_modularity(&graph, &same_community).unwrap();
        ...
        let diff_modularity = detector.calculate_modularity(&graph, &diff_community).unwrap();
    ...
    fn test_best_community_finding() -> Result<()> {
        ...
        let best_community = detector.find_best_community(
            &graph,
            nodes[0],
            &node_to_community,
            total_weight,
    ...
    fn test_different_resolution_parameters() {
        ...
        let low_res_modularity = low_res_detector.calculate_modularity(&graph, &node_to_community).unwrap();
        
        // Test with high resolution 
        let high_res_detector = LouvainDetector::new(1.5, 100, 1e-4);
        let high_res_modularity = high_res_detector.calculate_modularity(&graph, &node_to_community).unwrap();
