...
use crate::live::types::{CallEdgeEvent, AggregatedEdge, EdgeKind};
    ...
    pub async fn query_aggregated(&self, query: &AggregationQuery) -> Result<Vec<AggregatedEdge>> {
        ...
        while current_date <= end_date {
            for service in &query.services {
                let pattern = self.get_partition_pattern(service, current_date.and_hms_opt(0, 0, 0).unwrap().and_utc());
        ...
        all_edges.retain(|edge| {
            ...
            && (query.versions.is_empty() || 
                query.versions.iter().any(|v| 
                    edge.caller.contains(v) || edge.callee.contains(v)
                ))
        });
...
mod tests {
    ...
    async fn test_aggregation_query_date_filtering() {
        ...
        let query = AggregationQuery {
            ...
            edge_kinds: vec![crate::live::types::EdgeKind::Runtime, crate::live::types::EdgeKind::Static],
        };
    ...
    fn test_aggregation_bucket_multiple_services() {
        ...
        let worker_edges: Vec<_> = edges.iter().filter(|e| e.caller.contains("worker:")).collect();
