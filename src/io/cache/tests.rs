...
fn test_cache_expires_when_past_max_age() -> Result<()> {
    ...
    let refreshed_file: serde_json::Value = serde_json::from_str(&fs::read_to_string(&cache_path)?)
...
fn test_compute_codebase_signature_order_independent() {
    let temp_dir = tempdir().expect("Failed to create temp dir");
    let cache_manager = StopMotifCacheManager::new(temp_dir.path(), CacheRefreshPolicy::default());
