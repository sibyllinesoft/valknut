...
impl StopMotifCacheManager {
    ...
    fn get_valid_cache(&self, codebase_info: &CodebaseInfo) -> Result<Option<StopMotifCache>> {
        ...
        let cache_age = SystemTime::now()
            ...
            .as_secs() - cache.last_updated;
        
        let max_age_seconds = self.refresh_policy.max_age_days * 24 * 60 * 60;
        if cache_age > max_age_seconds {
            tracing::info!("Cache expired: {} days old (max: {} days)", 
                      cache_age / (24 * 60 * 60), self.refresh_policy.max_age_days);
            return Ok(None);
        }
        ...
        if cache.codebase_signature != current_signature {
            let change_percent = self.estimate_change_percentage(&cache.codebase_signature, &current_signature);
            if change_percent > self.refresh_policy.change_threshold_percent {
                tracing::info!("Codebase changed significantly: {:.1}% (threshold: {:.1}%)", 
                          change_percent, self.refresh_policy.change_threshold_percent);
                return Ok(None);
            }
        }
    ...
    fn refresh_cache(&self, codebase_info: &CodebaseInfo) -> Result<Arc<StopMotifCache>> {
        ...
        tracing::info!("Refreshing stop-motifs cache for {} functions", codebase_info.functions.len());
        ...
        *self.cache.write().unwrap() = Some(cache.clone());
        ...
        tracing::info!("Stop-motifs cache refreshed in {}ms: {} token grams, {} motifs", 
                  mining_duration, cache.token_grams.len(), cache.pdg_motifs.len());
    ...
    fn load_cache(&self, cache_path: &Path) -> Result<StopMotifCache> {
        let content = fs::read_to_string(cache_path)
            .map_err(|e| ValknutError::io(format!("Failed to read cache file: {}", cache_path.display()), e))?;
    ...
    fn save_cache(&self, cache: &StopMotifCache) -> Result<()> {
        ...
        fs::create_dir_all(&self.cache_dir)
            .map_err(|e| ValknutError::io(format!("Failed to create cache directory: {}", self.cache_dir.display()), e))?;
        ...
        fs::write(&temp_path, content)
            .map_err(|e| ValknutError::io(format!("Failed to write cache file: {}", temp_path.display()), e))?;
        
        // Atomic rename
        fs::rename(&temp_path, &cache_path)
            .map_err(|e| ValknutError::io(format!("Failed to rename cache file: {}", cache_path.display()), e))?;
...
impl PatternMiner {
    ...
    pub fn mine_stop_motifs(&mut self, codebase_info: &CodebaseInfo) -> Result<StopMotifCache> {
        ...
        tracing::info!("Mining patterns from {} functions", codebase_info.functions.len());
        ...
        tracing::info!("Pattern mining complete: {} unique k-grams, {} unique motifs, {} stop-motifs selected", 
                  mining_stats.unique_kgrams_found, mining_stats.unique_motifs_found, mining_stats.stop_motifs_selected);
        
        // Mine AST patterns using the new AST Stop-Motif Miner
        let mut ast_miner = AstStopMotifMiner::new();
        let ast_patterns = ast_miner.mine_ast_stop_motifs(&codebase_info.functions)
            .unwrap_or_else(|e| {
        ...
        updated_mining_stats.ast_node_types_found = ast_patterns.iter()
            .filter(|p| matches!(p.category, AstPatternCategory::NodeType))
            .count();
        updated_mining_stats.ast_subtree_patterns_found = ast_patterns.iter()
            .filter(|p| matches!(p.category, AstPatternCategory::SubtreePattern))
            .count();
        updated_mining_stats.languages_processed = ast_patterns.iter()
        ...
        Ok(StopMotifCache {
            ...
            token_grams: stop_motifs.clone().into_iter().filter(|e| e.category == PatternCategory::TokenGram).collect(),
            pdg_motifs: stop_motifs.into_iter().filter(|e| e.category != PatternCategory::TokenGram).collect(),
            ast_patterns,
    ...
    fn extract_all_patterns(&mut self, codebase_info: &CodebaseInfo) -> Result<()> {
        ...
        let kgram_freq: HashMap<String, usize> = codebase_info.functions
            .par_iter()
        ...
        let motif_freq: HashMap<String, usize> = codebase_info.functions
            .par_iter()
    ...
    fn extract_function_kgrams(&self, func: &FunctionInfo) -> HashMap<String, usize> {
        ...
        let tokens: Vec<String> = func.source_code
            .split_whitespace()
    ...
    fn select_stop_motifs(&self, idf_scores: &HashMap<String, f64>) -> Result<Vec<StopMotifEntry>> {
        ...
        let selection_count = ((all_patterns.len() as f64) * self.policy.stop_motif_percentile / 100.0).ceil() as usize;
    ...
    fn normalize_token(&self, token: &str) -> String {
        ...
        match token {
            ...
            "break" | "continue" | "return" | "yield" | "await" | "try" | "catch" |
            "finally" | "throw" | "with" => token.to_string(),
            
            // Function/class keywords - preserve for structural patterns
            "fn" | "function" | "def" | "class" | "struct" | "enum" | "trait" | 
            "interface" | "type" | "let" | "var" | "const" | "mut" | "pub" |
            "public" | "private" | "protected" | "static" => token.to_string(),
            ...
            _ => {
                ...
                if token.parse::<f64>().is_ok() {
                    ...
                    "STR_LIT".to_string()
                } else if token.len() < 20 && token.chars().all(|c| c.is_alphanumeric() || c == '_') &&
...
pub trait LanguageAdapter: Send + Sync {
    ...
    fn parse_source(&mut self, source_code: &str, file_path: &str) -> Result<crate::lang::common::ParseIndex>;
    fn extract_ast_patterns(&self, parse_index: &crate::lang::common::ParseIndex, source_code: &str) -> Result<Vec<AstPattern>>;
}
            ...
            if let Some(serde_json::Value::Array(params)) = entity.metadata.get("parameters") {
                if !params.is_empty() {
                    let param_pattern = AstPattern {
                        ...
                        node_type: None,
                        subtree_signature: Some(format!("function_definition->parameters[{}]", params.len())),
                        token_sequence: None,
                        language: "python".to_string(),
                        metadata: HashMap::new(),
                    };
    ...
    fn extract_js_token_sequences(&self, source_code: &str) -> Result<Vec<AstPattern>> {
        ...
        for line in source_code.lines() {
            ...
            for sequence in &common_js_sequences {
                if line.contains(sequence) {
                    let pattern = AstPattern {
                        id: format!("token_seq:{}", sequence.replace(" ", "_").replace("(", "").replace(")", "")),
                        pattern_type: AstPatternType::TokenSequence,
                        node_type: None,
                        subtree_signature: None,
                        token_sequence: Some(sequence.to_string()),
...
impl AstStopMotifMiner {
    ...
    pub fn new() -> Self {
        ...
        let config = AstExtractionConfig {
            ...
            enabled_languages: ["python", "javascript", "typescript", "rust", "go"].iter().map(|s| s.to_string()).collect(),
        };
    ...
    pub fn mine_ast_stop_motifs(&mut self, functions: &[FunctionInfo]) -> Result<Vec<AstStopMotifEntry>> {
                ...
                match adapter.parse_source(&function.source_code, &function.file_path) {
                    Ok(parse_index) => {
                        ...
                        match adapter.extract_ast_patterns(&parse_index, &function.source_code) {
                            ...
                            Err(e) => {
                                eprintln!("Failed to extract AST patterns from {}: {:?}", function.id, e);
                            }
                        }
                    }
                    Err(e) => {
        ...
        self.pattern_extractor.analyze_pattern_frequencies(&all_patterns);
        
        // Select stop-motifs based on frequency thresholds
        let stop_motifs = self.select_stop_motifs(&all_patterns)?;
        
        let duration = start_time.elapsed();
        println!("AST stop-motif mining completed in {:?}ms", duration.as_millis());
        println!("Found {} AST patterns, selected {} as stop-motifs", all_patterns.len(), stop_motifs.len());
        ...
        for (i, (pattern_id, support)) in frequency_pairs.iter().enumerate() {
            if let Some(pattern) = patterns.iter().find(|p| &p.id == pattern_id) {
                let percentile_threshold = match pattern.pattern_type {
                    ...
                    AstPatternType::ControlFlowPattern => self.frequency_thresholds.subtree_percentile,
                ...
                if pattern_percentile >= percentile_threshold && *support >= self.pattern_extractor.config.min_support {
                    // Calculate IDF score
            ...
            match &pattern.pattern_type {
                AstPatternType::NodeType => {
                    if let Some(ref node_type) = pattern.node_type {
                        *self.node_type_frequencies.entry(node_type.clone()).or_insert(0) += 1;
                    }
                }
                AstPatternType::SubtreePattern => {
                    if let Some(ref signature) = pattern.subtree_signature {
                        *self.subtree_frequencies.entry(signature.clone()).or_insert(0) += 1;
                    }
                }
                AstPatternType::TokenSequence => {
                    if let Some(ref sequence) = pattern.token_sequence {
                        *self.token_sequence_frequencies.entry(sequence.clone()).or_insert(0) += 1;
                    }
                }
...
mod tests {
    ...
    fn test_stop_motif_cache_serialization() {
        let cache = StopMotifCache {
            ...
            mining_stats: MiningStats {
                ...
                languages_processed: ["python".to_string(), "rust".to_string()].into_iter().collect(),
            },
        };
        ...
        let deserialized: StopMotifCache = serde_json::from_str(&json).expect("Failed to deserialize cache");
    ...
    fn test_pattern_miner_kgram_extraction() {
        ...
        let has_normalized = kgrams.keys().any(|k| 
            k.contains("LOCAL_VAR") || k.contains("INT_LIT") || k.contains("STR_LIT")
        );
    ...
    fn test_pattern_miner_motif_extraction() -> Result<()> {
        ...
        let has_control = motif_keys.iter().any(|k| k.contains("control:branch") || k.contains("control:loop"));
        let has_boilerplate = motif_keys.iter().any(|k| k.contains("boiler:debug_print") || k.contains("boiler:error_unwrap"));
    ...
    fn test_pattern_miner_stop_motif_selection() -> Result<()> {
        ...
        let codebase_info = CodebaseInfo {
            functions: vec![
                ...
                FunctionInfo {
                    id: "func2".to_string(),
                    source_code: "fn func2() { println!(\"test2\"); if x > 0 { process(); } }".to_string(),
    ...
    fn test_cache_invalidation_by_change() -> Result<()> {
        ...
        assert_ne!(sig1, sig2, "Signatures should differ for different codebases");
    ...
    fn test_motif_categorization() {
        ...
        assert_eq!(miner.categorize_motif("control:branch"), PatternCategory::ControlFlow);
        assert_eq!(miner.categorize_motif("control:loop"), PatternCategory::ControlFlow);
        assert_eq!(miner.categorize_motif("assign:assign"), PatternCategory::Assignment);
        assert_eq!(miner.categorize_motif("call:call"), PatternCategory::FunctionCall);
        assert_eq!(miner.categorize_motif("data:collection"), PatternCategory::DataStructure);
        assert_eq!(miner.categorize_motif("boiler:debug_print"), PatternCategory::Boilerplate);
        assert_eq!(miner.categorize_motif("boiler:error_unwrap"), PatternCategory::Boilerplate);
        assert_eq!(miner.categorize_motif("unknown:pattern"), PatternCategory::Boilerplate);
    }
