...
use crate::core::pipeline::{
    ...
    DirectoryHealthScore, DirectoryHealthTree, FeatureContribution, NormalizedAnalysisResults,
    NormalizedEntity, NormalizedIssue, NormalizedIssueTotals, NormalizedSummary,
...
#[test]
fn test_build_unified_hierarchy_handles_root_level_file_health() {
    let mut file_health = HashMap::new();
    file_health.insert("lib.rs".to_string(), 96.0);
    file_health.insert("src/main.rs".to_string(), 88.0);

    let mut directory_health = HashMap::new();
    directory_health.insert(".".to_string(), 92.0);
    directory_health.insert("src".to_string(), 88.0);

    let tree = DirectoryHealthTree::from_file_health(&file_health);
    let file_groups = create_file_groups_from_health(&file_health);
    let hierarchy =
        build_unified_hierarchy_with_health(&tree, &file_groups, &file_health, &directory_health);
    ...
    let children = root_node["children"]
        .as_array()
        .expect("root should contain children");
    assert!(children
        .iter()
        .any(|child| child["type"] == "file" && child["path"] == "lib.rs"));
    assert!(children
        .iter()
        .any(|child| child["type"] == "folder" && child["path"] == "src"));
}
...
fn test_add_files_to_hierarchy_enriches_metadata() {
    ...
    let enriched = add_files_to_hierarchy(&hierarchy, &file_groups, &dictionary, &candidate_lookup);
