...
fn evaluate_quality_gates_disabled_returns_health_score() {
    ...
    let gate =
        evaluate_quality_gates(&result, &config, false).expect("quality gate evaluation succeeds");
...
fn evaluate_quality_gates_handles_missing_metrics_when_verbose() {
    ...
    let gate =
        evaluate_quality_gates(&result, &config, true).expect("quality gate evaluation succeeds");
...
async fn run_analysis_with_progress_handles_denoise_configuration() -> Result<()> {
    ...
    let result = run_analysis_with_progress(&args.paths, StructureConfig::default(), &args).await?;
...
async fn run_analysis_without_progress_toggles_modules() -> Result<()> {
...

    {
        let _guard = DirGuard::change_to(&project_path);
        let summary =
            run_analysis_without_progress(&args.paths, StructureConfig::default(), &args).await?;
        assert!(
...
async fn test_preview_coverage_discovery_handles_absence() {
    ...
    let result = preview_coverage_discovery(&[temp_dir.path().to_path_buf()], &config, false).await;
    assert!(result.is_ok());
}
...
fn display_analysis_summary_prints_hotspots_and_metrics() {
    ...
    result
        ...
        .push(sample_candidate("src/utils.rs", Priority::Critical, 3.8));

    result
        .refactoring_candidates
        .push(sample_candidate("src/helpers/mod.rs", Priority::High, 2.9));
...
fn combine_analysis_results_merges_runs() {
    ...
    let expected_refactoring = first.summary.refactoring_needed + second.summary.refactoring_needed;
    ...
    assert!(combined.summary.code_health_score >= 0.5 && combined.summary.code_health_score <= 0.7);
