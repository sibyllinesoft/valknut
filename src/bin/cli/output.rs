...
fn refactoring_type_emoji(refactoring_type: &str) -> &'static str {
...
fn format_refactoring_type(refactoring_type: &str) -> String {
    refactoring_type
...
fn display_file_complexity_recommendations(file_result: &serde_json::Value) {
...
fn display_top_structure_issues(result: &serde_json::Value) {
...
fn format_location(rec: &serde_json::Value) -> String {
...
fn build_report_generator() -> anyhow::Result<ReportGenerator> {
...
async fn write_jsonl(result: &serde_json::Value, out_path: &Path) -> anyhow::Result<()> {
...
async fn write_ci_summary(result: &serde_json::Value, out_path: &Path) -> anyhow::Result<()> {
...
/// Render a single file entry in the high priority section.
fn render_md_file_entry(content: &mut String, file_result: &serde_json::Value, index: usize) {
    ...
    if let Some(issues) = file_result.get("issues").and_then(|v| v.as_array()) {
        content.push_str(&render_issues_markdown(issues, 5));
    }

    if let Some(recs) = file_result.get("recommendations").and_then(|v| v.as_array()) {
        content.push_str(&render_recommendations_markdown(recs, 3));
    }
    content.push('\n');
}

/// Render complexity summary statistics.
...
/// Render refactoring opportunities section.
fn render_md_refactoring_section(content: &mut String, result: &serde_json::Value) {
    ...
    let Some(count) = refactoring.get("opportunities_count").and_then(|v| v.as_u64()) else {
        return;
    };
...
/// Render the recommendations footer section.
...
/// Helper to determine CSS class for a metric based on its value and thresholds.
fn metric_class(value: f64, good_threshold: f64, warning_threshold: f64, higher_is_better: bool) -> &'static str {
    if higher_is_better {
...
/// Render a single metric card for the HTML report.
fn render_metric_card(title: &str, value: f64, class: &str, suffix: &str, note: Option<&str>) -> String {
    let note_html = note.map_or(String::new(), |n| format!("<small>{}</small>", n));
    format!(
        "<div class='metric-card {}'><h3>{}</h3><div class='metric-value'>{:.1}{}</div>{}</div>",
        class, title, value, suffix, note_html
    )
}

/// Render severity indicator for an issue.
fn severity_indicator(severity: &str) -> (&'static str, &'static str) {
    match severity {
...
fn effort_class(effort: u64) -> &'static str {
    match effort {
        1..=3 => "effort-low",
        4..=6 => "effort-medium",
        7..=10 => "effort-high",
...
fn render_issues_html(issues: &[serde_json::Value], limit: usize) -> String {
...
fn render_issues_markdown(issues: &[serde_json::Value], limit: usize) -> String {
...
fn render_recommendations_markdown(recommendations: &[serde_json::Value], limit: usize) -> String {
    if recommendations.is_empty() {
        return String::new();
    }
...
fn render_recommendations_html(recommendations: &[serde_json::Value], limit: usize) -> String {
    if recommendations.is_empty() {
        return String::new();
    }
...
fn extract_complexity_sonar_issues(complexity: &serde_json::Value) -> Vec<serde_json::Value> {
    let Some(detailed_results) = complexity.get("detailed_results").and_then(|v| v.as_array()) else {
        return Vec::new();
    };
...
fn build_sonar_issue_from_complexity(issue: &serde_json::Value, file_path: &str) -> serde_json::Value {
...
fn extract_refactoring_sonar_issues(refactoring: &serde_json::Value) -> Vec<serde_json::Value> {
    let Some(detailed_results) = refactoring.get("detailed_results").and_then(|v| v.as_array()) else {
        return Vec::new();
    };
...
fn build_sonar_issue_from_recommendation(rec: &serde_json::Value, file_path: &str) -> serde_json::Value {
    let priority_score = rec.get("priority_score").and_then(|v| v.as_f64()).unwrap_or(0.0);
...
fn build_sonar_issue(rule_key: &str, severity: &str, message: &str, file_path: &str, line: u64) -> serde_json::Value {
    serde_json::json!({
        "engineId": "valknut",
...
fn extract_complexity_csv_rows(complexity: &serde_json::Value) -> Vec<String> {
    let Some(detailed_results) = complexity.get("detailed_results").and_then(|v| v.as_array()) else {
        return Vec::new();
    };
...
fn extract_refactoring_csv_rows(refactoring: &serde_json::Value) -> Vec<String> {
    let Some(detailed_results) = refactoring.get("detailed_results").and_then(|v| v.as_array()) else {
        return Vec::new();
    };
...
fn build_refactoring_csv_row(rec: &serde_json::Value, file_path: &str) -> String {
    let refactoring_type = rec.get("refactoring_type").and_then(|v| v.as_str()).unwrap_or("Refactoring");
    let description = rec.get("description").and_then(|v| v.as_str()).unwrap_or("Refactoring opportunity");
    let priority_score = rec.get("priority_score").and_then(|v| v.as_f64()).unwrap_or(0.0);
...
fn extract_structure_csv_rows(structure: &serde_json::Value) -> Vec<String> {
