...
impl FeaturePrior {
    ...
    pub fn with_type(mut self, feature_type: impl Into<String>, distribution: impl Into<String>) -> Self {
...
impl BayesianNormalizer {
    ...
    pub fn fit(&mut self, feature_vectors: &[FeatureVector]) -> Result<()> {
        if feature_vectors.is_empty() {
            return Err(ValknutError::validation("No feature vectors provided for Bayesian fitting"));
        }
        ...
        for (feature_name, values) in feature_values {
            ...
            let prior = self.priors.get(&feature_name)
                .cloned()
            ...
            self.statistics.insert(feature_name.clone(), posterior_stats);
    ...
    fn normalize_value(&self, value: f64, stats: &FeatureStatistics) -> Result<f64> {
        ...
        let normalized = match self.scheme.as_str() {
            ...
            _ => {
                return Err(ValknutError::config(
                    format!("Unknown normalization scheme: {}", self.scheme)
                ));
            }
        };
    ...
    pub fn get_diagnostics(&self) -> HashMap<String, serde_json::Value> {
        ...
        let confidence_counts = self.variance_confidence
            .values()
        ...
        diagnostics.insert("confidence_distribution".to_string(), 
                          serde_json::to_value(confidence_counts).unwrap());
        
        let feature_count = self.statistics.len();
        diagnostics.insert("total_features".to_string(), 
                          serde_json::Value::Number(serde_json::Number::from(feature_count)));
        
        let avg_prior_weight: f64 = self.statistics.values()
            .map(|s| s.prior_weight)
            .sum::<f64>() / feature_count as f64;
        diagnostics.insert("average_prior_weight".to_string(), 
                          serde_json::Value::Number(
                              serde_json::Number::from_f64(avg_prior_weight).unwrap_or_else(|| serde_json::Number::from(0))
                          ));
...
mod tests {
    ...
    fn test_posterior_calculation() {
        ...
        let posterior = normalizer.calculate_posterior_stats(&empirical, &prior).unwrap();
