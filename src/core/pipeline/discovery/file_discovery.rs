...
/// Canonicalize root paths, falling back to original on failure.
...
/// Gather include, exclude, and ignore patterns from configuration.
fn gather_patterns(
...
/// Returns default patterns for commonly excluded directories.
...
/// Build set of allowed file extensions from configuration.
...
/// Compile glob patterns into an optimized GlobSet.
...
/// Find git repository and collect tracked files if available.
...
/// Collect tracked files from the git index.
...
/// Determine if a file should be kept based on filters and constraints.
fn should_keep(
    path: &Path,
    base: &Path,
    include_glob: Option<&GlobSet>,
...
/// Check if a path is within one of the requested root directories.
...
/// Get the default base directory for a path (its parent).
