...
/// Factory and analysis methods for [`CoverageStage`].
impl<'a> CoverageStage<'a> {
    ...
    async fn calculate_overall_coverage(
        ...
        for coverage_file in coverage_files {
            ...
            let Ok(content) = std::fs::read_to_string(&coverage_file.path) else {
                continue;
            };
            if let Some(percentage) = Self::parse_lcov_coverage(&content) {
    ...
    /// Parse LCOV content and return coverage percentage if valid.
    fn parse_lcov_coverage(content: &str) -> Option<f64> {
        let (total_lines, covered_lines) = Self::count_lcov_lines(content);
        if total_lines == 0 {
            return None;
        }
        ...
        Some(coverage_percentage)
    }

    /// Count total and covered lines from LCOV DA: entries.
    fn count_lcov_lines(content: &str) -> (usize, usize) {
        let mut total_lines = 0;
        let mut covered_lines = 0;

        for line in content.lines() {
            if let Some(covered) = Self::parse_lcov_da_line(line) {
                total_lines += 1;
                if covered {
                    covered_lines += 1;
                }
    ...
    /// Parse a single LCOV DA: line and return whether it's covered.
    fn parse_lcov_da_line(line: &str) -> Option<bool> {
        let data = line.strip_prefix("DA:")?;
        let parts: Vec<&str> = data.split(',').collect();
        if parts.len() < 2 {
            return None;
        }
        let hits: usize = parts[1].parse().ok()?;
        Some(hits > 0)
    }
