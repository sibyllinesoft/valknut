...
use crate::core::pipeline::clone_detection::{
    ...
    filter_small_pairs, get_or_build_simple_ast, hash_kind, ordered_pair_key, parse_byte_range,
    serialize_clone_pairs, should_skip_small_pair, CachedSimpleAst, CloneDetectionStats,
    CloneEndpoint, ClonePairReport, CloneVerificationDetail, LshDetectionParams,
    LshEntityCollection,
};
...
async fn coverage_stage_discovers_multiple_formats() {
    ...
    std::fs::write(
        &xml_path,
        "<coverage><line number=\"1\" hits=\"0\"/></coverage>",
    )
    .expect("write xml");
...
async fn analyze_coverage_gaps_skips_unknown_formats() {
    ...
    assert!(
        result.is_ok(),
        "coverage analysis should handle unknown formats gracefully"
    );
}
