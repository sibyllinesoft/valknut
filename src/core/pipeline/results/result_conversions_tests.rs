...
use crate::core::pipeline::results::pipeline_results::{
    LshAnalysisResults as PipelineLshAnalysisResult, MemoryStats as PipelineMemoryStats, TfIdfStats,
};
...
use crate::detectors::coverage::{
    CoverageGap, CoveragePack, FileInfo, GapFeatures, GapMarkers, GapSymbol, PackEffort, PackValue,
    SnippetPreview, SymbolKind, UncoveredSpan,
};
use chrono::Utc;
use serde_json::json;
...
fn test_refactoring_candidate_creation() {
    ...
    let candidate =
        RefactoringCandidate::from_scoring_result(&scoring_result, &[], std::path::Path::new(""));
...
fn from_pipeline_results_populates_dictionary_and_warnings() {
    ...
    let analysis =
        AnalysisResults::from_pipeline_results(pipeline_results, std::path::PathBuf::new());
...
fn convert_lsh_to_clone_analysis_returns_details() {
    ...
    let clone_analysis =
        AnalysisResults::convert_lsh_to_clone_analysis(&pipeline_results).expect("should convert");
