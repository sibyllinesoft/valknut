...
pub trait FeatureExtractor: Send + Sync {
    ...
    async fn extract(&self, entity: &CodeEntity, context: &ExtractionContext) -> Result<HashMap<String, f64>>;
            ...
            if let Some(definition) = self.get_feature_definition(name) {
                if !definition.is_valid_value(value) {
                    return Err(ValknutError::validation(
                        format!("Feature '{}' value {} is out of range", name, value)
                    ));
                }
            }
        }
...
impl FeatureExtractorRegistry {
    ...
    pub fn register(&mut self, extractor: Arc<dyn FeatureExtractor>) {
        ...
        for feature_def in extractor.features() {
            self.feature_definitions.insert(feature_def.name.clone(), feature_def.clone());
        }
...
mod tests {
    ...
    fn test_feature_validation() {
        let feature = FeatureDefinition::new("test", "Test feature")
            .with_range(0.0, 10.0);
    ...
    fn test_feature_vector_metadata() {
        ...
        vector.add_metadata("file_path", serde_json::Value::String("/path/to/file.rs".to_string()));
        
        assert_eq!(vector.metadata.get("language"), Some(&serde_json::Value::String("Rust".to_string())));
        assert_eq!(vector.metadata.get("file_path"), Some(&serde_json::Value::String("/path/to/file.rs".to_string())));
    }
    ...
    fn test_feature_vector_suggestions() {
        ...
        assert_eq!(vector.refactoring_suggestions[0].refactoring_type, "extract_method");
    }
    ...
    fn test_feature_vector_normalized_features() {
        let mut vector = FeatureVector::new("test_entity");
        vector.add_feature("complexity", 5.0);
        vector.normalized_features.insert("complexity".to_string(), 0.75);
    ...
    fn test_refactoring_suggestion_with_location() {
        let mut suggestion = RefactoringSuggestion::new(
            "extract_method",
            "Method too long",
            0.8,
            0.9,
    ...
    fn test_feature_extractor_validate_features() {
        ...
        extractor.add_feature(FeatureDefinition::new("valid_feature", "Valid")
    ...
    fn test_extraction_context() {
        ...
        let entity = CodeEntity::new("test_function_1", "function", "TestFunction", "test_file.rs");
        ...
        assert_eq!(context.context_data.get("language"), Some(&serde_json::Value::String("Rust".to_string())));
    }

    #[test]
    fn test_code_entity_with_source_code() {
        let mut entity = CodeEntity::new("test_function_1", "function", "TestFunction", "test_file.rs");
    ...
    fn test_code_entity_add_property() {
        ...
        entity.add_property("maintainability", serde_json::Value::String("high".to_string()));
        
        assert_eq!(entity.properties.get("complexity"), Some(&serde_json::Value::String("5".to_string())));
        assert_eq!(entity.properties.get("maintainability"), Some(&serde_json::Value::String("high".to_string())));
    }
    ...
    fn test_feature_extractor_registry_get_compatible_extractors() {
        ...
        let extractors: Vec<_> = registry.get_compatible_extractors(&entity).into_iter().collect();
