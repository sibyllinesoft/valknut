...
/// Factory and query methods for [`NodeMetadata`].
impl NodeMetadata {
...
/// Factory, detector registration, and traversal methods for [`UnifiedVisitor`].
impl UnifiedVisitor {
        ...
        while let Some((node, depth)) = stack.pop() {
            ...
            for detector in &mut self.detectors {
                let features = detector
                    ...
                    .await?;
                Self::merge_features(features, combined_features);
            }
    ...
    /// Merge features into combined map with conflict detection.
    fn merge_features(features: HashMap<String, f64>, combined: &mut HashMap<String, f64>) {
        for (name, value) in features {
            match combined.get(&name) {
                Some(&existing) if (existing - value).abs() > f64::EPSILON => {
                    ...
                    combined.insert(name, existing.max(value));
                }
                Some(_) => {} // Same value, no update needed
                None => {
...
/// Default implementation for [`UnifiedVisitor`].
impl Default for UnifiedVisitor {
    /// Returns a new unified visitor with no detectors.
    fn default() -> Self {
        Self::new()
    }
}
