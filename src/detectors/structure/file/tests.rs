...
fn test_lognormal_score_asymmetric() {
    ...
    assert!(
        score_at_optimal > score_below,
        "Score should decrease below optimal"
    );
    assert!(
        score_at_optimal > score_above,
        "Score should decrease above optimal"
    );
...
fn test_lognormal_score_at_95th_percentile() {
    ...
    assert!(
        score < 0.5,
        "Score at 95th percentile should be less than 0.5"
    );
}
...
"#;
    ...
    assert!(
        entity_health.entity_count >= 2,
        "Should find at least 2 entities"
    );
    assert!(
...
"#;
    ...
    let health = analyzer
        .calculate_entity_health(&file_path, &content)
        .unwrap();
...
fn test_is_entity_exported_handles_language_visibility() {
    ...
    assert!(!analyzer.is_entity_exported(&private_rust, Path::new("mod.rs"), "fn internal() {}"));
    ...
    assert!(analyzer.is_entity_exported(&top_level_unknown, Path::new("README.md"), "irrelevant"));
}
