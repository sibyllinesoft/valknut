...
impl StructureExtractor {
    ...
    pub async fn generate_recommendations(&self, root_path: &Path) -> Result<StructureRecommendations> {
    ...
    async fn generate_branch_reorg_packs(&self, root_path: &Path) -> Result<Vec<BranchReorgPack>> {
        ...
        let directories = self.directory_analyzer.discover_directories(root_path).await?;
        
        let packs: Vec<BranchReorgPack> = directories
            .iter()
            .filter_map(|dir_path| {
                self.directory_analyzer.analyze_directory_for_reorg(dir_path).ok().flatten()
            })
            .collect();
    ...
    async fn generate_file_split_packs(&self, root_path: &Path) -> Result<Vec<FileSplitPack>> {
        ...
        let packs: Vec<FileSplitPack> = large_files
            ...
            .filter_map(|file_path| {
                self.file_analyzer.analyze_file_for_split(file_path).ok().flatten()
            })
            .collect();
    ...
    pub fn calculate_directory_metrics(&self, dir_path: &Path) -> Result<DirectoryMetrics> {
        self.directory_analyzer.calculate_directory_metrics(dir_path)
    }
    ...
    pub fn analyze_directory_for_reorg(&self, dir_path: &Path) -> Result<Option<BranchReorgPack>> {
        self.directory_analyzer.analyze_directory_for_reorg(dir_path)
    }
    ...
    pub fn calculate_size_normalization_factor(&self, files: usize, total_loc: usize) -> f64 {
        self.directory_analyzer.calculate_size_normalization_factor(files, total_loc)
    }
}
