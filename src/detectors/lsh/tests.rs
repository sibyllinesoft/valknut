...
fn test_weighted_jaccard_similarity() {
    ...
    let sim_mismatch =
        analyzer.weighted_jaccard_similarity(&WeightedMinHashSignature::new(vec![1.0, 2.0]), &sig3);
...
fn test_generate_cache_key_is_order_insensitive() {
    ...
    let entity_b =
        CodeEntity::new("beta", "function", "beta", "beta.rs").with_source_code("fn beta() { 2 }");
