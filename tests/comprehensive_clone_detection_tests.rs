...
use valknut_rs::detectors::clone_detection::{
    ComprehensiveCloneDetector, TfIdfAnalyzer, PdgMotifAnalyzer, WeightedMinHash,
    NormalizationConfig
};
...
async fn test_tfidf_weighted_analysis() {
    ...
    analyzer.add_document("common_doc".to_string(), vec![
        "println".to_string(), "debug".to_string(), "common".to_string()
    ]);
    
    analyzer.add_document("rare_doc".to_string(), vec![
        "complex_algorithm".to_string(), "unique_function".to_string(), "rare".to_string()
    ]);
    
    analyzer.add_document("mixed_doc".to_string(), vec![
        "println".to_string(), "complex_algorithm".to_string(), "mixed".to_string()
    ]);
    ...
    assert!(rare_tfidf > common_tfidf, 
        "Rare terms should have higher TF-IDF scores: {} vs {}", rare_tfidf, common_tfidf);
...
async fn test_pdg_motif_analysis() {
    ...
    assert!(complex_motifs.len() > simple_motifs.len(),
...
async fn test_comprehensive_clone_detector() {
    ...
    context.entity_index.insert("entity1".to_string(), entity1.clone());
    context.entity_index.insert("entity2".to_string(), entity2.clone());
    ...
    assert!(*structural_evidence > 0.0, "Should detect structural similarity");
    
    // Final score should be calculated
    let final_score = features.get("final_clone_score").unwrap();
    assert!(*final_score > 0.0, "Should calculate meaningful clone score");
}
...
async fn test_adaptive_ranking_system() {
    ...
    let low_value_entity = CodeEntity::new(
        "low_value", 
        "function",
        "simple_getter",
        "utils.rs"
    ...
    let valknut_config = Arc::new(ValknutConfig::default());
    let mut context = ExtractionContext::new(valknut_config, "rust");
    context.entity_index.insert("high_value".to_string(), high_value_entity.clone());
    context.entity_index.insert("low_value".to_string(), low_value_entity.clone());
    
    // Test ranking - high-value entity should score higher
    let high_features = detector.extract(&high_value_entity, &context).await.unwrap();
    ...
    assert!(*high_score >= *low_score, 
        "Complex code should have higher clone detection score: {} vs {}", high_score, low_score);
    ...
    assert!(*high_rarity >= *low_rarity,
        "Complex code should have higher rarity gain: {} vs {}", high_rarity, low_rarity);
}
...
async fn test_structural_evidence_requirements() {
    ...
    let simple_entity = CodeEntity::new(
        "simple",
        "function",
        "simple",
        "test.rs"
    ...
    let complex_entity = CodeEntity::new(
        "complex",
        "function", 
        "complex",
        "test.rs"
    ...
    assert!(*complex_evidence > *simple_evidence,
...
async fn test_end_to_end_adaptive_detection() {
    ...
    let similar_evidence = similar_features.get("structural_evidence").unwrap();
    let boilerplate_evidence = boilerplate_features.get("structural_evidence").unwrap();
