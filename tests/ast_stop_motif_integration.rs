...
fn test_ast_stop_motif_mining_integration() {
    ...
    match result {
        Ok(ast_patterns) => {
            println!(
                "Successfully mined {} AST stop-motif patterns",
                ast_patterns.len()
            );

            // Verify we found some patterns
            assert!(
                !ast_patterns.is_empty(),
                "Should have found some AST patterns"
            );
            ...
            for (i, pattern) in ast_patterns.iter().take(5).enumerate() {
                println!(
                    "Pattern {}: {} (category: {:?}, support: {}, language: {})",
                    i + 1,
                    pattern.pattern,
                    pattern.category,
                    pattern.support,
...
fn test_ast_pattern_categories() {
    ...
    assert_eq!(
        token_sequence_pattern.category,
        AstPatternCategory::TokenSequence
    );
