...
fn test_memory_pool_effectiveness() {
    ...
    println!("  String reuse rate: {:.1}%", string_stats.reuse_rate() * 100.0);
    println!("  Signature reuse rate: {:.1}%", sig_stats.reuse_rate() * 100.0);
    ...
    assert!(string_stats.reused_count > 0 || sig_stats.reused_count > 0,
