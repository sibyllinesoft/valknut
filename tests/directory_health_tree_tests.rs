...
use valknut_rs::api::results::{
    AnalysisResults, AnalysisSummary, AnalysisStatistics, DirectoryHealthTree,
    FeatureContribution, MemoryStats, RefactoringCandidate,
    RefactoringIssue
};
...
mod directory_health_tree_tests {
    ...
    fn test_directory_health_tree_from_empty_candidates() {
        ...
        assert_eq!(health_tree.tree_statistics.total_directories, health_tree.directories.len());
    ...
    fn test_directory_health_tree_basic_structure() {
        let candidates = vec![
            create_test_candidate(
                "func1", "main_function", "src/main.rs", 
                Priority::High, 2.0, 0.9, vec![create_test_issue("complexity", "High complexity", 2.0)]
            ),
            create_test_candidate(
                "func2", "helper_function", "src/utils/helper.rs", 
                Priority::Medium, 1.5, 0.8, vec![create_test_issue("complexity", "High complexity", 2.0)]
            ),
            create_test_candidate(
                "func3", "api_handler", "src/api/handlers.rs", 
                Priority::Critical, 3.0, 0.95, vec![create_test_issue("complexity", "High complexity", 2.0)]
            ),
        ];
    ...
    fn test_directory_health_score_calculation() {
        ...
        let candidates = vec![
            create_test_candidate(
                "func1", "bad_function", "src/bad/terrible.rs", 
                Priority::Critical, 3.0, 0.95, vec![high_severity_issue]
            ),
            create_test_candidate(
                "func2", "ok_function", "src/good/decent.rs", 
                Priority::Low, 0.5, 0.6, vec![medium_severity_issue]
            ),
        ];

        let health_tree = DirectoryHealthTree::from_candidates(&candidates);
        
        let bad_dir = health_tree.directories.get(&PathBuf::from("src/bad")).unwrap();
        let good_dir = health_tree.directories.get(&PathBuf::from("src/good")).unwrap();
    ...
    fn test_directory_issue_summary_aggregation() {
        ...
        let candidates = vec![
            create_test_candidate(
                "func1", "func1", "src/module/file1.rs", 
                Priority::High, 2.0, 0.9, vec![complexity_issue1]
            ),
            create_test_candidate(
                "func2", "func2", "src/module/file2.rs", 
                Priority::Critical, 2.5, 0.95, vec![complexity_issue2]
            ),
            create_test_candidate(
                "func3", "func3", "src/module/file3.rs", 
                Priority::Medium, 1.5, 0.8, vec![structure_issue]
            ),
        ];

        let health_tree = DirectoryHealthTree::from_candidates(&candidates);
        let module_dir = health_tree.directories.get(&PathBuf::from("src/module")).unwrap();
    ...
    fn test_tree_statistics_calculation() {
        let candidates = vec![
            create_test_candidate(
                "func1", "func1", "src/level1/level2/file1.rs", 
                Priority::High, 2.0, 0.9, vec![create_test_issue("complexity", "High complexity", 2.0)]
            ),
            create_test_candidate(
                "func2", "func2", "src/level1/file2.rs", 
                Priority::Medium, 1.5, 0.8, vec![create_test_issue("complexity", "High complexity", 2.0)]
            ),
            create_test_candidate(
                "func3", "func3", "src/file3.rs", 
                Priority::Critical, 3.0, 0.95, vec![create_test_issue("complexity", "High complexity", 2.0)]
            ),
        ];
    ...
    fn test_hotspot_detection() {
        let candidates = vec![
            ...
            create_test_candidate(
                "func1", "terrible_func1", "src/hotspot/bad1.rs", 
                Priority::Critical, 3.0, 0.95, vec![
                    create_test_issue("complexity", "Very high complexity", 3.0),
                    create_test_issue("structure", "Very poor structure", 2.5),
                ]
            ),
            create_test_candidate(
                "func2", "terrible_func2", "src/hotspot/bad2.rs", 
                Priority::Critical, 2.8, 0.9, vec![
                    create_test_issue("complexity", "Very high complexity", 3.0),
                    create_test_issue("structure", "Very poor structure", 2.5),
                ]
            ),
            create_test_candidate(
                "func3", "terrible_func3", "src/hotspot/bad3.rs", 
                Priority::High, 2.5, 0.85, vec![
                    create_test_issue("complexity", "Very high complexity", 3.0),
                    create_test_issue("structure", "Very poor structure", 2.5),
                ]
            ),
            ...
            create_test_candidate(
                "func4", "good_func", "src/healthy/good.rs", 
                Priority::Low, 0.5, 0.6, vec![create_test_issue("complexity", "Low complexity", 0.5)]
            ),
        ];
        ...
        assert!(hotspot_paths.iter().any(|p| p.to_string_lossy().contains("hotspot")));
    ...
    fn test_tree_string_generation() {
        let candidates = vec![
            create_test_candidate(
                "func1", "func1", "project/src/main.rs", 
                Priority::Medium, 1.5, 0.8, vec![create_test_issue("complexity", "Medium complexity", 1.5)]
            ),
            create_test_candidate(
                "func2", "func2", "project/src/utils/helper.rs", 
                Priority::High, 2.0, 0.9, vec![create_test_issue("complexity", "Medium complexity", 1.5)]
            ),
            create_test_candidate(
                "func3", "func3", "project/tests/integration.rs", 
                Priority::Low, 0.8, 0.7, vec![create_test_issue("complexity", "Medium complexity", 1.5)]
            ),
        ];
        ...
        assert!(tree_string.contains("✓") || tree_string.contains("!") || tree_string.contains("⚠"));
        ...
        let indented_lines: Vec<&str> = lines.iter().filter(|line| line.starts_with("  ")).map(|s| *s).collect();
    ...
    fn test_get_children_method() {
        let candidates = vec![
            create_test_candidate(
                "func1", "func1", "src/utils/helper1.rs", 
                Priority::High, 2.0, 0.9, vec![create_test_issue("complexity", "High complexity", 2.0)]
            ),
            create_test_candidate(
                "func2", "func2", "src/utils/helper2.rs", 
                Priority::Medium, 1.5, 0.8, vec![create_test_issue("complexity", "High complexity", 2.0)]
            ),
            create_test_candidate(
                "func3", "func3", "src/api/handler.rs", 
                Priority::High, 2.0, 0.9, vec![create_test_issue("complexity", "High complexity", 2.0)]
            ),
        ];
    ...
    fn test_edge_case_single_file() {
        let candidates = vec![
            create_test_candidate(
                "func1", "standalone_function", "standalone.rs", 
                Priority::Medium, 1.5, 0.8, vec![create_test_issue("complexity", "High complexity", 2.0)]
            ),
        ];
        ...
        let root_key = health_tree.directories.keys().find(|k| k.to_string_lossy() == ".").cloned();
    ...
    fn test_edge_case_special_characters_in_paths() {
        let candidates = vec![
            create_test_candidate(
                "func1", "func_with_special_chars", "src/module-name/file_name.rs", 
                Priority::Medium, 1.5, 0.8, vec![create_test_issue("complexity", "High complexity", 2.0)]
            ),
            create_test_candidate(
                "func2", "func_with_numbers", "src/v2.0/handler123.rs", 
                Priority::High, 2.0, 0.9, vec![create_test_issue("complexity", "High complexity", 2.0)]
            ),
        ];
        ...
        let has_special_chars = health_tree.directories.keys()
    ...
    fn test_multiple_issues_per_entity() {
        ...
        let candidates = vec![
            create_test_candidate(
                "problematic_func", "very_problematic_function", "src/problems/bad.rs", 
                Priority::Critical, 3.0, 0.95, multiple_issues
            ),
        ];

        let health_tree = DirectoryHealthTree::from_candidates(&candidates);
        let problems_dir = health_tree.directories.get(&PathBuf::from("src/problems")).unwrap();
    ...
    fn test_depth_health_stats() {
        let candidates = vec![
            ...
            // Depth 2: src/level1
            create_test_candidate(
                "func2", "func2", "src/level1/file.rs", 
                Priority::Medium, 1.5, 0.8, vec![create_test_issue("complexity", "Medium complexity", 1.5)]
            ),
            // Depth 3: src/level1/level2
            create_test_candidate(
                "func3", "func3", "src/level1/level2/deep.rs", 
                Priority::High, 2.0, 0.9, vec![create_test_issue("complexity", "Medium complexity", 1.5)]
            ),
        ];
    ...
    fn test_analysis_results_directory_integration() {
        let candidates = vec![
            ...
            create_test_candidate(
                "func2", "func2", "src/utils/helper.rs", 
                Priority::High, 2.0, 0.9, vec![create_test_issue("complexity", "High complexity", 2.0)]
            ),
        ];
        ...
        assert!(hotspots.len() <= results.directory_health_tree.as_ref().unwrap().directories.len());
    ...
    fn test_json_serialization_with_directory_tree() {
        let candidates = vec![
            create_test_candidate(
                "func1", "test_function", "src/main.rs", 
                Priority::High, 2.0, 0.9, vec![create_test_issue("complexity", "High complexity", 2.0)]
            ),
        ];
        ...
        let deserialized: DirectoryHealthTree = serde_json::from_str(&json)
            .expect("Should deserialize from JSON");
        
        // Verify deserialized data matches original
        assert_eq!(deserialized.root.path, health_tree.root.path);
        assert_eq!(deserialized.directories.len(), health_tree.directories.len());
        assert_eq!(deserialized.tree_statistics.total_directories, 
        ...
        for (path, original_dir) in &health_tree.directories {
            let deserialized_dir = deserialized.directories.get(path)
    ...
    fn test_hotspot_recommendation_generation() {
        ...
        let candidates = vec![
            create_test_candidate(
                "func1", "complex_func", "src/complexity_hotspot/bad.rs", 
                Priority::Critical, 3.0, 0.95, complexity_issues
            ),
            create_test_candidate(
                "func2", "struct_func", "src/structure_hotspot/bad.rs", 
                Priority::High, 2.5, 0.9, structure_issues
            ),
            create_test_candidate(
                "func3", "coupled_func", "src/coupling_hotspot/bad.rs", 
                Priority::High, 2.2, 0.85, graph_issues
            ),
        ];
