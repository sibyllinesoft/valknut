...
fn test_lsh_optimization_performance() {
    ...
    for &count in &test_sizes {
        ...
        assert!(build_time.as_millis() < 1000, "Build time too slow: {:?}", build_time);
        assert!(search_time.as_millis() < 500, "Search time too slow: {:?}", search_time);
        
        // Verify we found some similarity candidates
        if count > 5 {
            assert!(total_candidates > 0, "Should find some similarity candidates");
        }
    }
}
...
fn test_signature_generation_performance() {
    ...
    for signature in &signatures {
        ...
        assert!(signature.iter().any(|&x| x != u64::MAX), "Signature should not be all MAX values");
    }
    ...
    println!("Average signature generation time: {}ms", avg_time_per_signature);
    assert!(avg_time_per_signature < 100, "Signature generation too slow: {}ms", avg_time_per_signature);
}
...
fn test_lsh_band_configuration() {
    ...
    for (num_hashes, num_bands) in configs {
        println!("\nTesting LSH config: {} hashes, {} bands", num_hashes, num_bands);
        ...
        let stats = context.get_statistics();
        println!("  Build time: {:?}, Search time: {:?}, Candidates: {}", 
                 build_time, search_time, candidates.len());
        println!("  Stats: {:?}", stats);
        
        // Validate configuration is working
        assert_eq!(stats.num_hashes, num_hashes);
        assert_eq!(stats.num_bands, num_bands);
        assert!(build_time.as_millis() < 2000, "Build time too slow for config");
        assert!(search_time.as_millis() < 100, "Search time too slow for config");
    }
}
...
fn test_memory_allocation_patterns() {
    ...
    let elapsed = start_time.elapsed();
    println!("Batch processed {} signatures in {:?}", all_signatures.len(), elapsed);
    ...
    assert!(avg_time < 50, "Batch processing too slow: {}ms average", avg_time);
}