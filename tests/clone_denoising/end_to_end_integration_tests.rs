...
use valknut_rs::core::config::{ValknutConfig, DedupeConfig, AdaptiveDenoiseConfig};
...
use valknut_rs::detectors::clone_detection::{
    ComprehensiveCloneDetector, StructuralGateAnalyzer, TfIdfAnalyzer, 
    AutoCalibrationEngine, PayoffRankingSystem, NormalizationConfig
};
use valknut_rs::io::cache::{StopMotifCacheManager, CacheRefreshPolicy};
...
mod end_to_end_pipeline_tests {
    ...
    async fn test_complete_4_phase_pipeline() {
        ...
        assert!(idf_result.is_ok(), "Phase 1: IDF table construction should succeed");
        
        // Compute weighted signatures  
        let weighted_signatures = weighted_analyzer.compute_weighted_signatures(&entity_refs).unwrap();
        assert_eq!(weighted_signatures.len(), test_entities.len(), "Phase 1: All entities should have signatures");
        ...
        assert!(entities_passing_gates.len() < test_entities.len(), 
               "Phase 2: Some entities should be filtered by structural gates");
        assert!(entities_passing_gates.len() > 0,
        ...
        let entities_after_cache_filtering = entities_passing_gates.into_iter()
            .filter(|entity| !entity.source_code.contains("println!") && !entity.source_code.contains("import os"))
            .collect::<Vec<_>>();
        
        assert!(entities_after_cache_filtering.len() <= test_entities.len(),
        ...
        let clone_candidates = comprehensive_detector.detect_clones_with_denoising(&entities_after_cache_filtering).await.unwrap();
        
        // Verify end-to-end results
        assert!(!clone_candidates.is_empty(), "E2E: Should detect some clone candidates");
        
        // Verify quality metrics are calculated
        for candidate in &clone_candidates {
            assert!(candidate.quality_metrics.overall_quality >= 0.0 && candidate.quality_metrics.overall_quality <= 1.0,
                   "E2E: Quality metrics should be in valid range");
            assert!(candidate.payoff_score >= 0.0, "E2E: Payoff scores should be non-negative");
        }
        
        // Verify ranking (highest payoff first)
        for i in 0..clone_candidates.len().saturating_sub(1) {
            assert!(clone_candidates[i].payoff_score >= clone_candidates[i+1].payoff_score,
    ...
    async fn test_cli_flag_integration() {
        ...
        assert!(results.clone_analysis.is_some(), "Should include clone analysis results");
        let clone_results = results.clone_analysis.unwrap();
        assert!(clone_results.denoising_enabled, "Denoising should be enabled");
        assert!(clone_results.candidates_before_denoising >= clone_results.candidates_after_denoising,
               "Denoising should reduce or maintain candidate count");
        
        // Test --auto-denoise flag
        let auto_denoise_config = ValknutConfig::default()
            .with_adaptive_denoise_config(AdaptiveDenoiseConfig {
                enabled: true,
        ...
        let mut auto_engine = ValknutEngine::new(auto_denoise_config.into()).await.unwrap();
        let auto_results = auto_engine.analyze_directory(&test_codebase_dir).await.unwrap();
        ...
        assert!(auto_clone_results.auto_calibration_applied, "Auto-calibration should be applied");
        assert!(auto_clone_results.calibrated_threshold > 0.0, "Should have calibrated threshold");
    }
...
"#,
        ...
        for test_case in test_cases {
            ...
            let phase1_result = phase1_analyzer.build_idf_table(&entities).is_ok() &&
                               phase1_analyzer.compute_weighted_signatures(&entities).is_ok();
            
            assert_eq!(phase1_result, test_case.expected_phase_results.phase1_weighted_signature,
                      "Phase 1 result mismatch for {}: expected {}, got {}", 
                      test_case.id, test_case.expected_phase_results.phase1_weighted_signature, phase1_result);
            
            if !phase1_result { continue; }
            
            // Phase 2: Structural Gates
            let phase2_analyzer = StructuralGateAnalyzer::new(2, 2);
            let phase2_result = phase2_analyzer.passes_structural_gates(&entity)
                .map(|r| r.passes_all_gates)
                .unwrap_or(false);
            
            assert_eq!(phase2_result, test_case.expected_phase_results.phase2_structural_gates,
                      "Phase 2 result mismatch for {}: expected {}, got {}",
                      test_case.id, test_case.expected_phase_results.phase2_structural_gates, phase2_result);
            
            if !phase2_result { continue; }
            ...
            if expected_phase3 { // Only check if we expect it to pass
                assert_eq!(phase3_result, expected_phase3,
    ...
    async fn test_cache_persistence_across_phases() {
        ...
        assert!(save_result.is_ok(), "Should save initial cache successfully");
        ...
        let new_signature = cache_manager.generate_codebase_signature(&create_codebase_info(&modified_entities));
        
        let should_refresh = cache_manager.should_refresh_cache(&cache, &new_signature);
        assert!(should_refresh, "Should trigger cache refresh for modified codebase");
        ...
        assert!(refresh_save_result.is_ok(), "Should save refreshed cache successfully");
        ...
        assert!(final_loaded_cache.last_updated > cache.last_updated,
    ...
    async fn test_quality_gate_effectiveness() {
        ...
        let clone_candidates = detector.detect_clones_with_denoising(&quality_test_entities).await.unwrap();
        ...
        assert!(high_quality_count > 0, "Should have some high-quality clones");
        assert!(low_quality_count <= clone_candidates.len() / 4, 
               "Low-quality clones should be minority: {} out of {}", low_quality_count, clone_candidates.len());
        
        // Test hard filtering floors effectiveness
        for candidate in &clone_candidates {
            assert!(candidate.saved_tokens >= 100, 
                   "All candidates should meet SavedTokens floor: {}", candidate.saved_tokens);
            assert!(candidate.rarity_gain >= 1.2,
                   "All candidates should meet RarityGain floor: {}", candidate.rarity_gain);
        }
        
        // Test ≥80% quality targeting  
        let quality_80_plus: Vec<_> = clone_candidates.iter()
            .filter(|c| c.quality_metrics.overall_quality >= 0.8)
            .collect();
        
        let quality_ratio = quality_80_plus.len() as f64 / clone_candidates.len() as f64;
        assert!(quality_ratio >= 0.6, // Allow some tolerance for test data
               "Should achieve reasonable proportion of ≥80% quality clones: {:.1}%", quality_ratio * 100.0);
    }
    ...
    fn create_comprehensive_test_codebase() -> Vec<CodeEntity> {
        vec![
            ...
            CodeEntity::new("matrix_multiply_v1", "function", "matrix_multiply", "/test/math1.py")
                .with_source_code(r#"
...
"#),
            ...
            CodeEntity::new("matrix_multiply_v2", "function", "multiply_matrices", "/test/math2.py")
                .with_source_code(r#"
...
"#),
            ...
            CodeEntity::new("unique_sort", "function", "adaptive_quicksort", "/test/unique.py")
                .with_source_code(r#"
...
"#),
    ...
    fn create_modified_test_codebase() -> Vec<CodeEntity> {
        vec![
            CodeEntity::new("modified_func", "function", "new_algorithm", "/test/modified.py")
                .with_source_code(r#"
...
"#.to_string(),
    ...
    fn create_test_stop_motif_cache(entities: &[CodeEntity]) -> valknut_rs::io::cache::StopMotifCache {
        use valknut_rs::io::cache::{StopMotifCache, StopMotifEntry, PatternCategory, MiningStats};
    ...
    fn create_codebase_info(entities: &[CodeEntity]) -> valknut_rs::io::cache::CodebaseInfo {
        ...
        CodebaseInfo {
            ...
            file_info: entities.iter().map(|entity| {
                FileInfo {
...
mod performance_integration_tests {
    ...
    async fn test_e2e_pipeline_performance() {
        ...
        let results = detector.detect_clones_with_denoising(&entity_refs).await.unwrap();
        
        let execution_time = start_time.elapsed();
        
        // Performance assertions
        assert!(execution_time.as_secs() < 30, "Pipeline should complete within 30 seconds for 100 entities");
        assert!(!results.is_empty(), "Should produce results with large dataset");
        
        // Verify scalability characteristics
        let entities_per_second = large_test_entities.len() as f64 / execution_time.as_secs_f64();
        assert!(entities_per_second > 3.0, "Should process at least 3 entities per second: {:.2}", entities_per_second);
    }
    ...
    async fn test_memory_efficiency() {
        ...
        let mut detector = ComprehensiveCloneDetector::new(DedupeConfig::default());
        let entity_refs: Vec<&CodeEntity> = test_entities.iter().collect();
        let _results = detector.detect_clones_with_denoising(&entity_refs).await.unwrap();
