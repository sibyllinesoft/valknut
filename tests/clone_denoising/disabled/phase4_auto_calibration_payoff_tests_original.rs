...
use valknut_rs::detectors::clone_detection::{
    AutoCalibrationEngine, CalibrationResult, CloneCandidate, HardFilteringFloors,
    PayoffRankingSystem, QualityMetrics, RankedCloneCandidate,
};
...
mod auto_calibration_engine_tests {
    ...
    fn test_recalibration_detection() {
        ...
        println!(
            "Stability metric: {}",
            engine.get_thresholds().stability_metric
        );
    }
...
mod payoff_ranking_system_tests {
    ...
    fn test_payoff_formula_calculation() {
        ...
        for (saved_tokens, rarity_gain, live_reach_boost, quality_score, confidence, expected) in
            test_cases
        {
    ...
    fn test_ranking_order_by_payoff() {
        ...
        let payoffs: Vec<f64> = ranking_result.iter().map(|r| r.payoff_score).collect();
...
mod simplified_quality_tests {
    ...
    mod integration_tests {
        ...
        async fn test_complete_calibration_ranking_pipeline() {
            ...
            assert!(!ranking_result.is_empty(), "Should have ranked candidates");
