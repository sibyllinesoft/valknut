...
use valknut_rs::detectors::lsh::{WeightedShingleAnalyzer, WeightedMinHashSignature};
...
mod weighted_shingle_analyzer_tests {
    ...
    fn test_idf_table_construction_basic() {
        ...
        let entity2 = CodeEntity::new(
            ...
            "rare_pattern",
            "/test/file2.py"
        ).with_source_code("def rare_pattern():\n    complex_algorithm()\n    return unique_computation()");
    ...
    fn test_tfidf_weighting_accuracy() {
        ...
        assert!(similarity_high > similarity_cross, 
        ...
        for k in [1, 3, 5, 9] {
            ...
            if !kgrams.is_empty() {
                ...
                for kgram in &kgrams {
                    ...
                    assert_eq!(tokens.len(), k, "K-gram should have {} tokens for k={}", k, k);
                }
    ...
    fn test_128_dimension_minhash_signature() {
        ...
        assert_eq!(signature.signature.len(), 128, "Signature should be exactly 128 dimensions");
        ...
        let unique_values: std::collections::HashSet<_> = signature.signature.iter()
            ...
            .collect();
        assert!(unique_values.len() > 1, "Signature should have diverse values");
        
        // Verify no NaN or infinite values
        for &value in &signature.signature {
            assert!(value.is_finite(), "Signature values should be finite: {}", value);
        }
    }
    ...
    fn test_empty_and_edge_cases() {
        ...
        let empty_entity = CodeEntity::new("empty", "function", "empty", "/test/empty.py")
            .with_source_code("");
        ...
        assert_eq!(signature.signature.len(), 0, "Empty source should produce empty signature");
        
        // Test very short source code
        let short_entity = CodeEntity::new("short", "function", "short", "/test/short.py")
            .with_source_code("x");
    ...
    fn test_idf_calculation_correctness() {
        ...
        let entity1 = CodeEntity::new("e1", "function", "e1", "/test/e1.py")
            .with_source_code("common gram");
        let entity2 = CodeEntity::new("e2", "function", "e2", "/test/e2.py")
            .with_source_code("common gram different");
        let entity3 = CodeEntity::new("e3", "function", "e3", "/test/e3.py")
            .with_source_code("rare gram");
    ...
    fn test_lsh_pipeline_integration() {
        ...
        for i in 0..10 {
            ...
            ).with_source_code(&format!("def func_{}():\n    print('hello')\n    value_{} = {}", i, i, i));
        ...
        assert_relative_eq!(consistency_sim, 1.0, epsilon = 1e-6, 
                           "Same entity should produce identical signatures");
    }
}
    ...
    proptest! {
        ...
        fn prop_weighted_jaccard_symmetry(
            ...
            assert_relative_eq!(sim12, sim21, epsilon = 1e-10, 
                               "Weighted Jaccard similarity should be symmetric");
        }
