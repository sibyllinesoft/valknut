...
use valknut_rs::detectors::clone_detection::{
    BasicBlockAnalyzer, MotifCategory, NormalizationConfig, PdgMotif, PdgMotifAnalyzer,
...
def conditional_func(x):
    ...
    return result
"#;
        let conditional_entity =
            CodeEntity::new("cond", "function", "conditional_func", "/test/cond.py")
        ...
        assert!(
            ...
            "Conditional code should have at least 2 basic blocks, got {}",
            conditional_blocks
        );
...
"#;
        ...
        assert!(
            loop_blocks >= 3,
            "Loop with conditional should have at least 3 basic blocks, got {}",
            loop_blocks
        );
    }
...
"#;
        ...
        let func1 =
            CodeEntity::new("f1", "function", "func1", "/test/f1.py").with_source_code(func1_code);
        let func2 =
            CodeEntity::new("f2", "function", "func2", "/test/f2.py").with_source_code(func2_code);
        ...
        assert!(
            overlap > 0.5,
            "Structurally similar functions should have >50% block overlap, got {}",
            overlap
        );
    ...
    fn test_minimum_block_threshold() {
        ...
        assert!(
            !passes_threshold,
            "Simple function should not pass minimum threshold"
        );
...
"#,
        ...
        assert!(
            passes_threshold,
            "Complex function should pass minimum threshold"
        );
    }
...
"#,
        ...
        assert_eq!(
            blocks, 1,
            "Function with only comments should have 1 basic block"
        );
...
"#,
        ...
        assert!(
            blocks >= 5,
            "Deeply nested function should have many basic blocks, got {}",
            blocks
        );
    }
}
...
"#;
        ...
        assert!(
            categories.contains(&MotifCategory::ControlFlow),
...
"#;
        ...
        let entity1 =
            CodeEntity::new("s1", "function", "similar1", "/test/s1.py").with_source_code(func1);
        let entity2 =
            CodeEntity::new("s2", "function", "similar2", "/test/s2.py").with_source_code(func2);
        ...
        assert!(
            common_hashes > 0,
            "Similar structures should share some WL hashes"
        );
    }
...
"#;
        ...
        assert!(
            shared_count >= 2,
            "Functions with similar structure should share ≥2 motifs, got {}",
            shared_count
        );
...
"#;
        ...
        let diff_entity =
            CodeEntity::new("diff", "function", "different_structure", "/test/diff.py")
                .with_source_code(diff_func_code);

        let shared_count_diff = analyzer
            .count_shared_motifs(&entity1, &diff_entity)
            .unwrap();
        assert!(
            shared_count_diff < shared_count,
            "Different structures should share fewer motifs: {} vs {}",
            shared_count_diff,
            shared_count
        );
    }
...
"#;
        ...
        assert!(
            categories.contains(&MotifCategory::Assignment),
            "Should detect assignment motifs"
        );
        assert!(
            categories.contains(&MotifCategory::ControlFlow),
            "Should detect control flow motifs"
        );
        assert!(
            categories.contains(&MotifCategory::FunctionCall),
            "Should detect function call motifs"
        );
        assert!(
            categories.contains(&MotifCategory::DataStructure),
            "Should detect data structure motifs"
        );
    }
}
...
"#;
        ...
        let valid_entity = CodeEntity::new(
            "valid",
            "function",
            "valid_clone_processing",
            "/test/valid.py",
        )
        .with_source_code(valid_clone_func);

        let gate_result = gate_analyzer
            .passes_structural_gates(&valid_entity)
            .unwrap();
        assert!(
            gate_result.passes_all_gates,
            "Valid clone should pass all structural gates"
        );
        assert!(
            gate_result.block_count >= 2,
            "Should meet minimum block requirement"
        );
        assert!(
            gate_result.motif_count >= 2,
            "Should meet minimum motif requirement"
        );
        ...
        let gate_result = gate_analyzer
            .passes_structural_gates(&invalid_entity)
            .unwrap();
        assert!(
            !gate_result.passes_all_gates,
            "Simple function should fail structural gates"
        );
    }
...
"#;
        ...
        assert!(
            gate_result.io_penalty_factor > 1.0,
            "IO-heavy function should have penalty factor > 1.0"
        );
        assert!(
            !gate_result.io_patterns.is_empty(),
            "Should detect IO patterns"
        );
...
"#;
        ...
        assert!(
            pure_gate_result.io_penalty_factor <= gate_result.io_penalty_factor,
...
"#;
        ...
        let external_entity = CodeEntity::new(
            "external",
            "function",
            "external_heavy_function",
            "/test/ext.py",
        )
        .with_source_code(external_calls_func);

        let gate_result = gate_analyzer
            .passes_structural_gates(&external_entity)
            .unwrap();

        // Should detect external call patterns
        assert!(
            !gate_result.external_call_patterns.is_empty(),
            "Should detect external call patterns"
        );
        assert!(
            gate_result.external_call_penalty > 1.0,
            "Should apply penalty for external calls"
        );
...
"#;
        ...
        assert!(
            result1.block_count > result2.block_count,
...
"#;
        ...
        assert!(
            gate_result.passes_all_gates,
            "Complex function should pass all gates"
        );
...
"#,
            ...
            (
                "fail2",
                "def single_expression(data): return sum(x.value for x in data if x.is_valid())",
            ),
        ];
