...
fn test_analyze_code_tool_integration() {
    ...
    assert!(results_path.exists(), "Analysis results file should be created");
    
    // Read and validate the analysis results
    let content = std::fs::read_to_string(&results_path).expect("Should be able to read results file");
...
fn test_analysis_without_pre_existing_results() {
    ...
    let results_path = project_path.join(".valknut").join("analysis_results.json");
    assert!(results_path.exists(), "Should create analysis results from scratch");
...
mod real_mcp_protocol_tests {
    ...
    async fn test_mcp_tools_list() {
        ...
        stdin.write_all((tools_msg.to_string() + "\n").as_bytes()).await.unwrap();
        stdin.flush().await.unwrap();
        ...
        match result {
            Ok(_) => {
                ...
                assert!(analyze_tool["input_schema"]["required"].as_array().unwrap().contains(&json!("path")));
                
                // Validate get_refactoring_suggestions tool
                let refactor_tool = tools.iter()
    ...
    async fn test_mcp_refactoring_suggestions_tool_call() {
        ...
        match result {
            ...
            Err(_) => {
                panic!("MCP server should respond to get_refactoring_suggestions within 30 seconds");
            }
        }
    ...
    async fn test_mcp_analyze_code_different_formats() {
        ...
        for format in formats {
            ...
            match result {
                        ...
                        match format {
                            "json" => {
                                ...
                                let _: Value = serde_json::from_str(text)
                                    .expect(&format!("JSON format should produce valid JSON, got: {}", text));
                            }
                            "markdown" => {
                            ...
                            "html" => {
                                ...
                                assert!(text.contains("<html>") || text.contains("<body>") || text.contains("<div>"), 
                ...
                Err(_) => {
                    panic!("MCP server should respond within 30 seconds for format: {}", format);
                }
            }
    ...
    async fn test_mcp_error_handling() {
        ...
        stdin.write_all((invalid_version.to_string() + "\n").as_bytes()).await.unwrap();
        stdin.flush().await.unwrap();
        
        let mut response = String::new();
        timeout(Duration::from_secs(5), reader.read_line(&mut response)).await.unwrap();
        ...
        stdin.write_all((unknown_method.to_string() + "\n").as_bytes()).await.unwrap();
        stdin.flush().await.unwrap();
        ...
        stdin.write_all((invalid_path_call.to_string() + "\n").as_bytes()).await.unwrap();
        stdin.flush().await.unwrap();
        
        timeout(Duration::from_secs(10), reader.read_line(&mut response)).await.unwrap();
        
        let response_json: Value = serde_json::from_str(&response.trim()).unwrap();
        assert!(response_json["error"].is_object());
        assert_eq!(response_json["error"]["code"], -32602); // INVALID_PARAMS
        assert!(response_json["error"]["message"].as_str().unwrap().contains("does not exist"));
        ...
        stdin.write_all((unknown_tool.to_string() + "\n").as_bytes()).await.unwrap();
        stdin.flush().await.unwrap();
    ...
    async fn test_mcp_concurrent_requests() {
        ...
        for i in 10..13 {
            ...
            let mut response = String::new();
            timeout(Duration::from_secs(30), reader.read_line(&mut response)).await
...
mod mcp_protocol_unit_tests {
    ...
    fn test_refactoring_suggestions_schema_validation() {
        ...
        assert_eq!(max_suggestions_prop["description"], "Maximum number of suggestions");
    }
...
mod mcp_integration_validation {
    ...
    fn test_mcp_integration_completeness() {
        ...
        server_cmd.timeout(std::time::Duration::from_secs(3))
            .assert()
            .stderr(predicate::str::contains("📡 Starting MCP stdio server"));
    }
    ...
    fn test_mcp_analysis_integration() {
        ...
        let results_path = test_project.path().join(".valknut").join("analysis_results.json");
        assert!(results_path.exists() || test_project.path().join(".valknut").exists(),
