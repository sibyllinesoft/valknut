...
fn test_caching_effectiveness() {
    ...
    assert_eq!(
        ...
        "Cached signature should match original"
    );
...
fn test_token_caching_with_variations() {
    ...
    assert!(
        cache_stats.token_hits > 0,
        "Should have token cache hits for repeated source"
    );
}
...
fn test_cache_memory_management() {
    ...
    println!(
        "Generating signatures for {} different sources",
        sources.len()
    );
    ...
    for (i, source) in sources.iter().enumerate() {
        ...
        if i % 20 == 0 {
            let cache_stats = lsh_extractor.get_cache_statistics();
            println!(
                "After {} sources - hits: {}, misses: {}, evictions: {}",
                i + 1,
    ...
    assert!(
        final_stats.signature_hits > 0 || final_stats.token_hits > 0,
