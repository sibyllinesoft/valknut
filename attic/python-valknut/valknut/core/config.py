...
class RootConfig(BaseModel):
    ...
    exclude: list[str] = Field(
        ...
        description="Exclude patterns for performance optimization"
    )
...
class StructureConfig(BaseModel):
    """Configuration for filesystem structure analysis."""
    
    enable_branch_packs: bool = Field(default=True, description="Enable branch reorganization packs")
    enable_file_split_packs: bool = Field(default=True, description="Enable file splitting packs")
    top_packs: int = Field(default=20, ge=1, description="Maximum structure packs to generate")
    
    # Directory thresholds
    max_files_per_dir: int = Field(default=25, ge=1, description="Maximum files per directory")
    max_subdirs_per_dir: int = Field(default=10, ge=1, description="Maximum subdirectories per directory")
    max_dir_loc: int = Field(default=2000, ge=100, description="Maximum LOC per directory")
    min_branch_recommendation_gain: float = Field(default=0.15, ge=0.0, le=1.0, description="Minimum gain for branch recommendations")
    
    # File thresholds
    huge_loc: int = Field(default=800, ge=100, description="LOC threshold for huge files")
    huge_bytes: int = Field(default=128000, ge=1000, description="Byte threshold for huge files")
    
    # Analysis thresholds
    min_analysis_threshold_files: int = Field(default=5, ge=1, description="Minimum files to analyze directory")
    min_analysis_threshold_loc: int = Field(default=600, ge=100, description="Minimum LOC to analyze directory")
...
class ImpactPacksConfig(BaseModel):
    ...
    enable_coverage_packs: bool = Field(default=True, description="Enable coverage improvement packs")
...
class RefactorRankConfig(BaseModel):
    ...
    coverage_report_path: Optional[str] = Field(
        default=None, 
        description="Path to coverage report file (auto-detected if not provided)"
    )
    coverage_format_hint: Optional[str] = Field(
        default=None,
        description="Coverage report format hint: coverage.py, lcov, jacoco, cobertura, istanbul"
    )
