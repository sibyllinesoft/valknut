...
class Pipeline:
    ...
    def __init__(self, config: RefactorRankConfig) -> None:
        ...
        self.impact_pack_builder = ImpactPackBuilder(
            ...
            enable_coverage_packs=config.impact_packs.enable_coverage_packs,
            enable_structure_packs=config.structure.enable_branch_packs or config.structure.enable_file_split_packs,
            ...
            coverage_format_hint=config.coverage_format_hint,
    ...
    def _initialize_extractors(self) -> None:
        ...
        feature_registry.register(RefactoringAnalyzer())
    ...
    async def analyze(self) -> PipelineResult:
        ...
        try:
            ...
            impact_packs = await self._generate_impact_packs(parse_indices, discovered_files)
    ...
    async def _parse_and_index(self, files: List[Path]) -> Dict[str, ParseIndex]:
        ...
        # Parse each language group concurrently
        async def parse_language_group(language, language_files):
            ...
            try:
                ...
                if cached_index:
                    ...
                    index = await loop.run_in_executor(None, adapter.parse_index, language_files)
        ...
        # Run all language parsing concurrently
        tasks = [
            parse_language_group(language, language_files) 
    ...
    async def _extract_features(self, indices: Dict[str, ParseIndex]) -> List[FeatureVector]:
        """Extract features from all entities sequentially to avoid echo conflicts."""
        all_feature_vectors = []
        
        # Process each language index sequentially to avoid echo concurrency issues
        for language, index in indices.items():
            ...
            # Extract features for each entity (keeping sequential for now due to echo)
    ...
    async def _generate_impact_packs(self, parse_indices: Dict[str, ParseIndex], files: List[Path] = None) -> List[ImpactPack]:
        ...
        # Build impact packs (including structure analysis)
