...
use valknut_rs::detectors::lsh::{LshExtractor, LshConfig};
use std::sync::Arc;
...
fn benchmark_complexity_comparison(c: &mut Criterion) {
    ...
    for &count in &entity_counts {
        ...
        group.bench_with_input(
            BenchmarkId::new("lsh_optimized", count),
            &count,
            |b, _| {
                b.iter(|| {
            ...
            |b, _| {
                b.iter(|| {
                    for entity in &entities {
                        let _signature = lsh_extractor.generate_minhash_signature(&entity.source_code);
                    }
                })
            },
        );
...
fn benchmark_lsh_throughput(c: &mut Criterion) {
    ...
    for &count in &entity_counts {
        ...
        let optimized_extractor = LshExtractor::new()
            .with_lsh_config(LshConfig {
                num_hashes: 128,
                num_bands: 16,
                shingle_size: 3,
        ...
        group.bench_with_input(
            ...
            |b, _| {
                b.iter(|| {
                    ...
                    let context = optimized_extractor.create_similarity_search_context(&entities_refs);
                    ...
                    for i in 0..count.min(20) {  // Test with subset for timing
                        let entity_id = format!("func_{}", i);
                    ...
                    black_box((build_time, search_time, total_candidates, context.get_statistics()))
                })
            },
        );
    }
