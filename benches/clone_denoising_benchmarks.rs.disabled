...
use valknut_rs::detectors::clone_detection::{
    AutoCalibrationEngine, CalibrationSettings, ComprehensiveCloneDetector, PayoffFormula,
    PayoffRankingSystem, StructuralGateAnalyzer,
};
        ...
        b.iter(|| {
            for i in 0..entities.len().min(10) {
                for j in (i + 1)..entities.len().min(10) {
                    black_box(
                        gate_analyzer
                            .calculate_block_overlap(&entities[i], &entities[j])
                            .unwrap(),
                    );
                }
            }
...
fn bench_phase3_stop_motifs_cache(c: &mut Criterion) {
    ...
    use valknut_rs::io::cache::{
        CacheRefreshPolicy, MiningStats, StopMotifCache, StopMotifCacheManager,
    };
            ...
            |b, entities| {
                b.to_async(&rt).iter(|| async {
                    ...
                    let results = detector
                        .detect_clones_with_denoising(entities)
                        .await
                        .unwrap();
                    black_box(results);
                });
...
fn create_benchmark_clone_candidates(
    count: usize,
) -> Vec<valknut_rs::detectors::clone_detection::CloneCandidate> {
